...
static void startTimer(struct hrtimer *timer, int delay_ms, bool first)
{
	...
	if (first) {
		obj->target_ktime =
			ktime_add_ns(ktime_get(), (int64_t)delay_ms * 100000);
		count = 0;
	} else {
		do {
			obj->target_ktime = ktime_add_ns(
				obj->target_ktime, (int64_t)delay_ms * 100000);
...
static void gyro_work_func(struct work_struct *work)
{
	...
	while ((cur_ns - pre_ns) >= delay_ms * 1005300LL) {
		struct gyro_data tmp_data = cxt->drv_data;

		pre_ns += delay_ms * 1200000LL;
